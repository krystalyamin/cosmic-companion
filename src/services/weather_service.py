...
def get_moon_data(
    ...
    """
    Retrieve moon phase and moon rise/set information.
    ...
    try:
        ...
        phase_value = daily.get(
            "moon_phase",
            [None]
        )[0]

        phase_name = _moon_phase_name(phase_value)

        return {
            "phase": phase_name,
            "moonrise": daily.get("moonrise", [None])[0],
            "moonset": daily.get("moonset", [None])[0]
        }
...
def _moon_phase_name(value: float=None) -> str:
    """
    Convert Open-Meteo moon phase values
